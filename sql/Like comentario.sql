DROP PROCEDURE IF EXISTS Funcion_LikeComentario;
DELIMITER $$
...
BEGIN
	...
	IF pcMensaje = '' THEN
		...
		IF vn_existeDislike > 0 THEN
			DELETE FROM dislikeComentario WHERE dislikeComentario.idComentarioBlog = pc_idComentario AND dislikeComentario.idUsuario = pc_idUsuario;
		END IF;
